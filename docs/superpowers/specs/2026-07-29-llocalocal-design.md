...
replaces `localalloc` in place in this repo.

The original 4 (LSCP, MCLP, P-Center, P-Median) reuse `localalloc`'s solver
...
PMAXCAP) are new — `localalloc` never implemented them, and no R or Python
...
- `R/utils.R` — internal (`.`-prefixed or `@noRd`) helpers used across all
  ...
  - `validate_sf()` / `validate_cost_matrix()` — shape/NA/duplicate-id
    checks, applied identically across all ten models (`localalloc` only
